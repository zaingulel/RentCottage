...
# and CI lint it.
#
# One convention for a deliberate pass, shared with the Codex twin:
...
#   exit 0, stated reason — could not verify (no project root resolved, an unenterable root, a root
#                           git cannot inspect, a required tool absent, or lint unable to evaluate):
#                           the applicable observation did not complete, and stderr says so; a product
#                           gate that could not verify states its own reason and exits 0
#   exit 2, stated reason — verified and failed, including a product gate that exited non-zero or is
#                           not executable
# The unenterable-root exit code deliberately diverges from the Codex twin's exit 2: a root this
# twin resolved from `git rev-parse` is the repository the hook is already inside, so in practice
# that branch is only reached for the CLAUDE_PROJECT_DIR fallback — an environment value that may
# simply be stale (a deleted or renamed worktree), which is could-not-verify, not verified-failed.
input=$(cat)

# Avoid the Stop-hook infinite loop: if we already forced one continuation, let it stop.
case "$input" in *'"stop_hook_active"'*true*) exit 0 ;; esac

# Resolve the repository this hook is actually running in first, exactly as the Codex twin does:
# CLAUDE_PROJECT_DIR stays at the session's original project root, so for a session working inside
# a worktree the root checkout is clean, `-- src` matched nothing, and the gate passed on exactly
# the change it exists to check. The environment value remains the fallback for a run with no
# work tree under it.
root=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$root" ] || root="$CLAUDE_PROJECT_DIR"
# POSIX `cd ""` SUCCEEDS, so an empty (not merely unset) value would slide past the `||` guard
# below and leave the gate running working-directory-relative, where its pathspecs match nothing.
if [ -z "$root" ]; then
...
# `git status … 2>/dev/null` alone cannot distinguish a clean tree from git FAILING (not a work
# tree, git absent, dubious ownership) — and a failure must not read as the silent verified pass.
if ! pending=$(git status --porcelain -- src 2>/dev/null); then
  echo "Stop gate: git could not inspect the resolved project root, so the lint observation is unavailable." >&2
  exit 0
fi
[ -n "$pending" ] || exit 0
