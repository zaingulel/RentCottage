...
The text conversation for one booking journey that a phone-verified customer may start from a Cottage Profile before requesting and continue through the Booking Request and Confirmed Booking. Payment retries and recovery for that request retain the same conversation. A later independent booking has a separate conversation, with its own payment permission and writing deadline; an earlier paid conversation does not unlock contact sharing in a new one. Before Payment Capture, the system blocks phone numbers, email addresses, web links and social handles so the parties cannot move the transaction off-platform. Phone-number detection covers Western, Arabic and Persian digits plus common spaces, dashes, brackets and country-code formats; repeated bypass attempts are flagged for administrator review. After confirmation, direct contact information may be shared. The conversation becomes read-only 30 days after the Booking Period ends, and its original and translated messages remain available to authorised participants and support.
