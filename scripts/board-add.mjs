...
// Run: node scripts/board-add.mjs <issue#> <Status> <routing value>
...
export function main(args = process.argv.slice(2), dependencies = {}) {
  ...
  try {
    if (args.length !== 3) {
      throw new Error(`usage: node scripts/board-add.mjs <issue#> <Status> <${ROUTING_FIELD}>`);
    }
    ...
    output.log(`board-add: Status = ${result.statusName}, ${ROUTING_FIELD} = ${result.workstreamName}`);
    return 0;
  } catch (err) {
