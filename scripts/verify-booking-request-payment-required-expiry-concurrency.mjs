...
const harness = createLocalSupabaseConcurrencyHarness({
  timing: {
    check: "verify-booking-request-payment-required-expiry-concurrency",
    isolation: "serial",
  },
});
...
try {
  ...
  const delayedObservationSql = async (permit, occurrence) => {
    harness.markTimingPhase("setup");
    try {
  ...
  const parsed = async (sql) =>
    JSON.parse(
      await harness.runSqlAfterSetup(paymentEvidenceSql, serviceSql(sql)),
    );
  const dueSql = (limit = 1) =>
    `select public.claim_due_booking_request_payment_required_expiries(${limit},${literal(identity)});`;
  const prepareSql = async (id = requestId, explicitCommand) => {
    const current = await facts(id);
  ...
  const prepare = async (id) => await parsed(await prepareSql(id));
  const releaseSql = async (permit, outcome = "succeeded") => {
    const current = await facts(permit.binding.bookingRequestId);
    const owned = current.expiryOperations.find(
  ...
  const recoverySql = async (permit, outcome = "succeeded") => {
    const command = await observationCommand(permit, outcome);
  ...
  const facts = async (id = requestId) =>
    await parsed(`select public.get_booking_request_payment_facts('${id}');`);
  const lease = async (id) => {
    const selected = selectPaymentRecovery(await facts(), id);
  ...
  const observationCommand = async (permit, outcome) => {
    const current = await facts(permit.binding.bookingRequestId);
    const operation = current.operations.find(
  ...
  const execute = async (permit, outcome) =>
    await parsed(await recoverySql(permit, outcome));
  ...
  const admit = async (command) =>
    JSON.parse(
      (
        await harness.runSqlAfterSetup(
          paymentEvidenceSql,
          authenticated(admitSql(command)),
        )
      )
        .split("\n")
        .at(-1),
...
from foreign_capture_admission cross join foreign_capture_occurrence;`,
  ...
  async function atDeadline(offset = "") {
    await harness.runSqlAfterSetup(
      paymentEvidenceSql,
  ...
  async function assertExpired(successfulCaptures = 0) {
    const state = await graph();
  ...
  async function assertAvailability(released) {
    ...
    const availability = await parsed(
  ...
  async function assertOverlap(released) {
    const sql = `begin;
    ...
    rollback;`;
    if (released) await harness.runSqlAfterSetup(paymentEvidenceSql, sql);
    else
      await assert.rejects(
        async () => await harness.runSqlAfterSetup(paymentEvidenceSql, sql),
  ...
  try {
    ...
    assert.deepEqual(
      await parsed(otherDueSql),
      [],
      "A matching locked request is skipped",
    );
    assert.deepEqual(
      (await parsed(dueSql())).map((row) => row.bookingRequestId),
      [requestId],
    ...
    const otherBatch = await start(
      `begin;${serviceSql(dueSql())}commit;`,
      true,
    );
    ...
    const failedRelease = await prepare();
    await parsed(await releaseSql(failedRelease.permit, "failed"));
    assert.equal((await parsed(finalizeSql())).status, "quarantined");
    assert.deepEqual(
      (await parsed(dueSql())).map((row) => row.bookingRequestId),
      [secondRequestId],
      "A previously evaluated blocked request cannot starve a later healthy request",
    );
    const healthyRelease = await prepare(secondRequestId);
    await parsed(await releaseSql(healthyRelease.permit));
    assert.equal(
      (await parsed(finalizeSql(secondRequestId))).status,
      "expired",
    );
    await assertHeld();
    console.log(
    ...
    assert.equal((await graph()).targets.length, 1);
    await assertAvailability(false);
    await assertOverlap(false);
    await proveRequestFirst(await releaseSql(permit), "release");
    const releasedPair = await race(
      await releaseSql(permit),
      await releaseSql(permit),
      "release_duplicate",
    );
    ...
    const afterRelease = await graph();
    assert.deepEqual(
      await parsed(await releaseSql(permit)),
      result(releasedPair[0]),
    );
    assert.deepEqual(await graph(), afterRelease);
    ...
    await assertExpired();
    await assertAvailability(true);
    await assertOverlap(true);
    assert.deepEqual(
      (await graph()).capture,
      unchanged.capture,
    ...
    assert.deepEqual(
      (await graph()).inventory,
      unchanged.inventory,
      "Expiry retains selected inventory history",
    );
    const completedGraph = await graph();
    assert.equal((await parsed(finalizeSql())).status, "expired");
    await parsed(await releaseSql(permit, "failed"));
    assert.deepEqual(
      await graph(),
      completedGraph,
    ...
    await race(
      ...
      "admit_deadline",
      {
        ...
        afterHeld: async () => await atDeadline(),
        expectedState: "RC409",
      },
    );
    assert.equal((await graph()).attempts.length, 0);
    await assertHeld();
    console.log(
    ...
    await reset();
    const authorizing = (await admit()).attemptId;
    await execute((await lease(authorizing)).permit);
    const authorizationPermit = (await lease(authorizing)).permit;
    const authorizedFirst = await race(
      await recoverySql(authorizationPermit),
      await prepareSql(),
      "authorization_wins",
      { afterHeld: async () => await atDeadline() },
    );
    ...
    for (const step of ["replacement-authorization", "replacement-capture"]) {
      await reset();
      const attempt = (await admit()).attemptId;
      await execute((await lease(attempt)).permit);
      if (step === "replacement-capture")
        await execute((await lease(attempt)).permit);
      const stalePermit = (await lease(attempt)).permit;
      assert.equal(stalePermit.step, step);
      await atDeadline();
      const pair = await race(
        await prepareSql(),
        await recoverySql(stalePermit),
        `stale_${step}`,
      );
    ...
    for (const expiryWins of [false, true]) {
      ...
      await execute((await lease(attempt)).permit);
      await execute((await lease(attempt)).permit);
      await execute((await lease(attempt)).permit, "failed");
      const recoveryRelease = (await lease(attempt)).permit;
      await atDeadline();
      const pair = expiryWins
        ? await race(
            ...
            await recoverySql(recoveryRelease),
            "release_expiry_wins",
          )
        : await race(
            await recoverySql(recoveryRelease),
      ...
      await assertExpired();
      assert.equal(
        (await graph()).ledger.filter((row) => row.operation_kind === "release")
          .length,
        2,
      );
      assert.equal(
        (await graph()).targets.filter(
          (target) =>
    ...
    for (const step of [
      ...
      for (const expiryWins of [false, true]) {
        ...
        for (
          let preceding = 0;
          preceding < 3 && (await lease(attempt)).permit.step !== step;
          preceding++
        ) {
          const next = (await lease(attempt)).permit;
          await execute(
            next,
            next.step === "replacement-capture" ? "failed" : "succeeded",
          );
        }
        const uncertainPermit = (await lease(attempt)).permit;
        assert.equal(uncertainPermit.step, step);
        const entry = await race(
          await recoverySql(uncertainPermit, "indeterminate"),
        ...
        assert.equal((await graph()).expiry.state, "quarantined");
        const uncertain = result(entry[0]);
        await atDeadline();
        const query = querySql(uncertainPermit, "succeeded", uncertain);
        const quarantined = await graph();
        const pair = await race(
          expiryWins ? await prepareSql() : query,
          expiryWins ? query : await prepareSql(),
        ...
        await assertHeld();
        assert.equal((await prepare()).status, "quarantined");
        assert.equal((await parsed(query)).outcome, "not-executed");
        assert.deepEqual(await graph(), quarantined);
      }
    }
    console.log(
    ...
    await reset();
    const preDeadline = (await admit()).attemptId;
    await execute((await lease(preDeadline)).permit);
    await execute((await lease(preDeadline)).permit);
    await execute((await lease(preDeadline)).permit);
    await atDeadline();
    const beforeConfirmation = await graph();
    ...
    assert.equal((await prepare()).status, "confirmed");
    assert.equal((await parsed(finalizeSql())).status, "confirmed");
    assert.equal((await parsed(dueSql())).length, 0);
    console.log(
    ...
    for (const offset of ["", "+ interval '1 second'"]) {
      ...
      const pair = await race(
        ...
        await prepareSql(
          ...
          literal({
            action: "refund",
            captureId: (await graph()).ledger.find(
              (entry) =>
      ...
      assert.equal((await parsed(finalizeSql())).status, "processing");
      await assertHeld();
      assert.equal((await graph()).attempts[0].state, "late_succeeded");
      assert.equal(
        (await graph()).targets.filter(
      ...
      assert.equal(
        (await graph()).targets.find(
          (operation) => operation.operation_kind === "refund",
        ).amount_fils,
        115000000,
      );
      await assert.rejects(async () => await parsed(confirmSql(attempt)));
      const refund = result(pair[1]).permit;
      await race(
        await releaseSql(refund),
        await releaseSql(refund),
        `refund_duplicate_${offset ? "after" : "equal"}`,
      );
      await drain();
      await assertExpired(1);
      const effects = (await graph()).ledger;
      assert.equal(
    ...
    for (const observationWins of [true, false]) {
      ...
      await execute((await lease(attempt)).permit);
      await execute((await lease(attempt)).permit);
      const permit = (await lease(attempt)).permit;
      const lateObservation = await delayedObservationSql(
        permit,
        `(select payment_required_deadline from public.booking_request_capture_work where booking_request_id='${requestId}')`,
      );
      await atDeadline();
      await parsed(lateObservation);
      await prepare(); // Adopt the explicitly known original recovery release before selecting the outstanding refund.
      const refund = await prepare();
      assert.equal(refund.status, "refund");
      const capture = (await graph()).ledger.find(
        (operation) =>
          operation.operation_kind === "capture" &&
          operation.current_outcome === "succeeded",
      );
      const observation = contradictoryObservation(capture);
      const dispatch = await releaseSql(refund.permit);
      const pair = await race(
      ...
      assert.equal((await parsed(dispatch)).outcome, "not-executed");
      assert.equal((await parsed(finalizeSql())).status, "quarantined");
      assert.deepEqual(
        await graph(),
        quarantined,
    ...
    for (const observationWins of [true, false]) {
      ...
      assert.equal((await parsed(confirmation)).status, "quarantined");
      assert.deepEqual(
        await graph(),
        state,
    ...
    await reset();
    const firstGeneration = (await admit()).attemptId;
    for (const outcome of ["succeeded", "succeeded", "failed", "succeeded"])
      await execute((await lease(firstGeneration)).permit, outcome);
    const secondGeneration = (
      await admit("81000000-0000-4000-8000-000000001002")
    ).attemptId;
    await execute((await lease(secondGeneration)).permit, "failed");
    await admit("81000000-0000-4000-8000-000000001003");
    const generations = await graph();
    await atDeadline();
    await race(await prepareSql(), finalizeSql(), "all_generations");
    await drain();
    await assertExpired();
    assert.deepEqual((await graph()).attempts, generations.attempts);
    assert.deepEqual((await graph()).operations, generations.operations);
    assert.equal((await graph()).targets.length, 2);
    assert.equal(
      (await graph()).targets.filter((target) => target.owner === "recovery")
        .length,
      2,
    );
    console.log(
