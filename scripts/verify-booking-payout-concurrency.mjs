...
const harness = createLocalSupabaseConcurrencyHarness({
  ...
  timing: { check: "verify-booking-payout-concurrency", isolation: "serial" },
});
...
try {
  ...
  try {
    await build({
      entryPoints: [
        new URL("./booking-payout-process-fixture.ts", import.meta.url)
          .pathname,
      ],
    ...
    const refundSetup = `begin;${fixture("PAYMENT EVIDENCE FIXTURE")}`;
    let refundBoth = "";
    ...
    await harness.runSqlAfterSetup(refundSetup, `${refundBoth}commit;`);
    ...
    assert.deepEqual(JSON.parse(await child("duplicate")), {
      status: "settled",
    });
    assert.equal(
      harness.runSql(
