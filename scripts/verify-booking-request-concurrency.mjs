...
function prepareBoundaryAttempt({
  ...
  if (authorize) {
    ...
    with prepared as (
      ...
      select result, snapshot,
        public.save_booking_request_payment_snapshot(
          (result ->> 'attemptId')::uuid,
          snapshot,
          '{"provider":"test-payments","environment":"test","merchantId":"concurrency-merchant","terminalId":"concurrency-terminal"}'::jsonb
        )
      from claimed
      where claim_result ->> 'status' = 'ready'
    )
    update public.test_booking_request_time_boundary_fixture fixture
    set attempt_id = (saved.result ->> 'attemptId')::uuid,
      payment_snapshot = saved.snapshot
    from saved
...
async function main() {
  ...
  const counts = runSql(`
    select
      ...
      (select count(*) from public.booking_request_submission_attempts
        where customer_user_id = '${customerId}'
          and authorization_provider_request_id = 'concurrency-provider-request');
  `);
  if (counts !== "1|1|1|1|1") {
    throw new Error(
  ...
  console.log(
    "Booking Request concurrency passed: effects=1|1|1|1|1 (attempt|request|Pending Hold|owner notice|provider authorization); payment-CAS=stale-save-blocked-then-RC409; claim-protected-price=RC204; preauth-six-hour-lock=blocked-across-boundary-then-too-late-without-authorization; preauth-inside-48-hour-lock=blocked-across-boundary-then-invalid-without-authorization; postauth-six-hour-lock=blocked-across-boundary-then-RC409-with-releasable-authorization; postauth-inside-48-hour-lock=blocked-across-boundary-then-RC409-with-releasable-authorization; cross-cottage-overlap=blocked-on-customer-lock-then-RC409; authoritative lookup=waits-then-absent-on-rollback-and-pending-on-commit; same-intent contender=blocked-then-pending.",
  );
}
