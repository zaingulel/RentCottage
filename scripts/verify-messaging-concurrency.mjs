...
try {
  ...
  await harness.waitForMarker(bookingOpenHolder, "MESSAGING_BOOKING_OPEN_HELD");
  ...
  await harness.waitForMarker(paymentHolder, "MESSAGING_PAYMENT_REQUEST_HELD");
  ...
  const committedBeforeQuarantine = JSON.parse(resultLine(duringPaymentChange));
  assert.equal(
