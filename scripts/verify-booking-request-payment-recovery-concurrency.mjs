...
const harness = createLocalSupabaseConcurrencyHarness({
  timing: {
    check: "verify-booking-request-payment-recovery-concurrency",
    isolation: "serial",
  },
});
...
try {
  ...
  const parsed = async (sql) => JSON.parse(await service(sql));
  ...
  const executeSql = async (permit, outcome = "succeeded") => {
  ...
  const execute = async (permit, outcome) =>
    await parsed(await executeSql(permit, outcome));
  ...
  async function duplicate(command, label) {
    const sql = typeof command === "function" ? await command() : command;
  ...
  const atDeadline = async (offset = "") =>
    await clock(`'${await deadline()}'::timestamptz ${offset}`);
  ...
  try {
    ...
    assert.equal((await admit()).attemptId, admission.attemptId);
    for (const step of [
      "original-release",
      "replacement-authorization",
      "replacement-capture",
    ]) {
      const work = await lease(admission.attemptId);
      assert.equal(work.permit.step, step);
      await duplicate(async () => await executeSql(work.permit), step);
    }
    ...
    await execute((await lease(failed.attemptId)).permit);
    await execute((await lease(failed.attemptId)).permit);
    await execute((await lease(failed.attemptId)).permit, "failed");
    await assert.rejects(
      async () => await admit("81000000-0000-4000-8000-000000001002"),
    );
    await execute((await lease(failed.attemptId)).permit);
    assert.equal((await lease(failed.attemptId)).status, "retryable");
    const retry = await admit("81000000-0000-4000-8000-000000001002");
    assert.equal(
      (await lease(retry.attemptId)).permit.step,
      "replacement-authorization",
    );
    assert.equal(
      await harness.runSqlAfterSetup(
    ...
    const deadlineContender = await start(
    ...
    await assert.rejects(async () => await admit());
    await atDeadline("- interval '1 second'");
    const beforeDeadline = await admit();
    await execute((await lease(beforeDeadline.attemptId)).permit);
    await execute((await lease(beforeDeadline.attemptId)).permit);
    await execute((await lease(beforeDeadline.attemptId)).permit);
    await atDeadline("+ interval '1 second'");
    assert.equal(
    ...
    assert.equal((await snapshot()).confirmation, null);
    const finalized = await parsed(finalizeSql(beforeDeadline.attemptId));
    assert.deepEqual(
      await parsed(finalizeSql(beforeDeadline.attemptId)),
      finalized,
    );
    ...
    for (const boundary of ["", "+ interval '1 second'"]) {
      ...
      await execute((await lease(late.attemptId)).permit);
      await execute((await lease(late.attemptId)).permit);
      const pending = await lease(late.attemptId);
      await assert.rejects(
        async () => await admit("81000000-0000-4000-8000-000000001002"),
      );
      const observation = await delayedObservationSql(
        pending.permit,
        `'${await deadline()}'::timestamptz ${boundary}`,
      );
      await atDeadline(boundary);
      assert.equal((await parsed(observation)).status, "recorded");
      await duplicate(observation, `late_${boundary ? "after" : "equal"}`);
      assert.equal((await lease(late.attemptId)).status, "late-succeeded");
      await assert.rejects(
        async () => await parsed(finalizeSql(late.attemptId)),
      );
      assert.equal((await snapshot()).confirmation, null);
      assert.equal((await snapshot()).physical, 3);
      assert.equal(
        await harness.runSqlAfterSetup(
          paymentEvidenceSql,
