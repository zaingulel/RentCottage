...
try {
  ...
  harness.runSql(
    `${fixture} select pg_temp.prepare_request_notice('request_new','cottage_owner'); commit;`,
  );
  ...
  harness.runSql(
    `${fixture.slice(fixture.indexOf("create function pg_temp.request_payload"))} select pg_temp.prepare_request_notice('request_accepted','cottage_owner');`,
  );
  const pending = parse(
    harness.runSql(
  ...
  for (const [action, status] of [
    ...
    const result = parse(
      harness.runSql(
        `${seed} ${terminalReleaseSql} select pg_temp.finish_terminal_release('${action}'); commit;`,
      ),
    );
    ...
    const delivered = parse(
      harness.runSql(
        `${helpers} select jsonb_agg(pg_temp.request_notice_call('execute',pg_temp.prepare_request_notice('request_${status}',role))) from (values ('customer'),('cottage_owner')) recipients(role);`,
      ),
    );
    assert.deepEqual(
