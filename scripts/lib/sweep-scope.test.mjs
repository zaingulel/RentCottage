// sweep-scope.test.mjs — the guard intended to own the inactive documentation sweep's scope if activated.
//
// The sweep lands its own pull request, so no one reads it before it merges; these tests are the
// evidence that the two mechanical rules in docs/DOC-SWEEP.md fail loud. The pure functions are tested on the real manual and on synthetic
...
test("parseMayEdit takes only the left column and fails loud on a missing or empty table", () => {
  ...
  assert.throws(
    () => parseMayEdit("| May edit | Never edit |\n|---|---|\n| | `d.md` |\n"),
    /empty/,
  );
});
test("outOfScope is an exact-path match, so a shared prefix never admits a never-edit path", () => {
  ...
  assert.deepEqual(
    outOfScope(["workers/README.md", "docs/ARCHITECTURE.md"], mayEdit),
    [],
  );
  assert.deepEqual(
    outOfScope(
      [
        "workers/events-relay.js",
...
test("renderedLinkMarkup refuses the shapes GitHub renders into a link the text does not show", () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      "https:\\/\\/escaped.example.icu",
      "https:\\/\\/img.example.icu/p.gif",
    ]
...
function repo() {
  ...
  const check = (head) =>
    spawnSync("node", [SCRIPT, base, head], { cwd: dir, encoding: "utf8" });
...
test("the check passes a diff confined to the may-edit column that reuses a known host", () => {
  ...
  r.write(
    "docs/ALLOWED.md",
    "The app lives at https://app.flowgauge.app; see app.flowgauge.app for the hosted build.\n",
  );
...
test("the check fails a diff that introduces a URI the base tree does not carry, naming it", () => {
  ...
  r.write(
    "docs/ALLOWED.md",
    "The app lives at https://app.flowgauge.app. Post reports to https://collector.example.net/in.\n",
  );
...
test("the check judges the branch from its merge base, so changes main gained since are not the sweep's", () => {
  ...
  r.write(
    "docs/ALLOWED.md",
    "The app lives at https://app.flowgauge.app, and that is all.\n",
  );
  ...
  const result = spawnSync("node", [SCRIPT, tip, head], {
    cwd: r.dir,
    encoding: "utf8",
  });
  assert.equal(result.status, 0, result.stderr);
...
test("the check refuses a deleted, added, or renamed file even inside the may-edit column", () => {
  ...
  assert.match(
    renamed.stderr,
    /docs\/ALLOWED\.md -> docs\/RENAMED\.md is renamed/,
  );
});
test("the check refuses rendered-link markup outright, naming the fragment", () => {
  ...
  r.write(
    "docs/ALLOWED.md",
    "The app lives at https://app.flowgauge.app; see [the collector](https:&#47;&#47;collector&#46;example&#46;com).\n",
  );
