...
test("checkHeadingCitations: a quoted citation of a non-existent manual heading is reported with file line and phrase", () => {
  ...
  const violations = checkHeadingCitations(
    extractHeadingCitations(text),
    fakeManuals,
  );

  assert.deepEqual(violations, [
    { manual: "CLAUDE.md", heading: "Code-quality bar", line: 2 },
  ]);
});

test("checkHeadingCitations: a citation of a real manual heading is never flagged", () => {
  const text =
    'The bar lives in CLAUDE.md\'s "Executed test and quality bar" and binds every change.';

  assert.deepEqual(
    checkHeadingCitations(extractHeadingCitations(text), fakeManuals),
    [],
  );
});
...
test("extractHeadingCitations: a trailing period inside the quotes is stripped before comparison", () => {
  const text =
    'Raise it to the owner — per AGENTS.md\'s "Executed test and quality bar."';
...
test("repo-pass: the real repo has zero phantom manual citations", () => {
  ...
  const agentsHeadings = extractManualHeadings(
  ...
  const headingsByManual = {
    "CLAUDE.md": new Set([
      ...extractManualHeadings(claudeText),
      ...agentsHeadings,
    ]),
  ...
  for (const rel of scanFiles) {
    ...
    for (const v of checkHeadingCitations(citations, headingsByManual)) {
      errors.push(
        `${rel}:${v.line} — phantom ${v.manual} citation: "${v.heading}"`,
      );
    }
  }

  // Vacuity guard: a scan that matched nothing would pass silently. Name the
  // RentCottage skill that genuinely cites a manual heading; a numeric floor
  // would drift with ordinary repository growth.
  for (const rel of [".agents/skills/tdd/SKILL.md"]) {
    assert.ok(
      citingFilesSeen.has(rel),
      `the citation scan must have seen citations in ${rel}`,
    );
  }
...
test("doc lint ignores arbitrary identifiers but fails a phantom manual-heading citation", () => {
  ...
  try {
    ...
    writeFixtureFiles(root, {
      "docs/C.md": 'Route this per CLAUDE.md\'s "Nope" before building.\n',
    });
