// sweep-scope.mjs — pure logic for the future hosted check over the inactive documentation sweep.
//
...
export function parseMayEdit(manualText) {
  ...
  const header = lines.findIndex(
    (line) => line.trim() === "| May edit | Never edit |",
  );
  if (header < 0)
    throw new Error(
      "docs/DOC-SWEEP.md has no scope table headed `| May edit | Never edit |`",
    );
  const paths = [];
  for (
    let i = header + 2;
  ...
  if (paths.length === 0)
    throw new Error("the scope table's may-edit column is empty");
  return paths;
}
...
const TLDS =
  "com|org|net|io|dev|app|ai|co|uk|eu|us|ca|au|de|fr|nl|ch|se|me|info|biz|xyz|top|site|online|" +
...
export function addedNetworkTokens(unifiedDiff) {
  ...
  for (const text of addedLines(unifiedDiff)) {
    ...
    const uris = [...text.matchAll(URI)].map((m) =>
      m[0].replace(/[.,;:_*]+$/, ""),
    );
    tokens.push(...uris);
    let rest = text;
    ...
    tokens.push(
      ...[...rest.matchAll(HOST)].map((m) => m[0].replace(/^\*?\./, "")),
    );
  }
...
const CHARACTER_REFERENCE =
  /&(?:#x?[0-9a-f]+|(?!(?:amp|lt|gt|quot|nbsp);)[a-z]+);/gi;
