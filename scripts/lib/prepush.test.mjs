...
function withFixture(options, fn) {
  const scratch = mkdtempSync(join(tmpdir(), "rentcottage-prepush-"));
  try {
...
test("pre-push invokes installed lint exactly once, then the script suite, from the resolved root", () => {
  withFixture({}, ({ recorded, repo, run }) => {
    ...
    assert.deepEqual(
      calls.map(([tool, args]) => `${tool} ${args}`),
      ["npm run lint --silent", "node --test scripts/lib/*.test.mjs"],
    );
    assert.deepEqual(
      calls.map(([, , cwd]) => cwd),
      [repo, repo],
...
test("pre-push refuses a completed red lint before the script suite", () => {
  withFixture({}, ({ recorded, run }) => {
    ...
    assert.deepEqual(
      recorded().map((line) => line.split("\t")[0]),
      ["npm"],
    );
  });
});
