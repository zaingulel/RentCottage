...
const NODE_SUMMARY =
  /^ℹ (tests|suites|pass|fail|cancelled|skipped|todo|duration_ms) /;
...
export function rewriteTestCommand(command) {
  ...
  return (
    `${cd ? cd[0] : ""}__fg_out=$(mktemp); ${runner} >"$__fg_out" 2>&1; __fg_status=$?; ` +
...
export function filterRunnerOutput(outputText, exitStatus) {
  ...
  const spec = lines.filter(
    (line) => NODE_SUMMARY.test(line) || PLAYWRIGHT_SUMMARY.test(line),
  );
  const summary =
    spec.length > 0 ? spec : lines.filter((line) => TAP_SUMMARY.test(line));
