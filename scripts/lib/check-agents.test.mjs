...
test("a missing or unclosed frontmatter fence is a named problem", () => {
  assert.match(
    checkAgentSource("reviewer.md", GOOD_MD.replace(/^---\n/, ""))[0],
...
test("an unquoted colon inside a plain scalar is the reviewer-vanished bug class", () => {
  const bad = GOOD_MD.replace(
    'description: "Adversarial pre-merge review: correctness, honesty, dead code."',
    "description: Adversarial review: correctness",
  );
  assert.match(
    checkAgentSource("reviewer.md", bad).join("\n"),
    /unquoted colon/,
  );
});

test("a block scalar may contain colons freely, and a plain key after it is still parsed", () => {
  const block = GOOD_MD.replace(
    'description: "Adversarial pre-merge review: correctness, honesty, dead code."\n',
  ...
  const plain = GOOD_MD.replace(
    'description: "Adversarial pre-merge review: correctness, honesty, dead code."',
    "description: Read: the diff first.",
  );
  assert.match(
    checkAgentSource("reviewer.md", plain).join("\n"),
...
test("missing required keys, a mismatched name, and unknown model or effort are named", () => {
  ...
  assert.match(
    checkAgentSource("critic.md", GOOD_MD).join("\n"),
    /name `reviewer` != filename `critic`/,
  );
  assert.match(
    checkAgentSource(
      "reviewer.md",
      GOOD_MD.replace("model: opus", "model: gpt-9"),
    ).join("\n"),
    /unknown model `gpt-9`/,
  );
  assert.match(
    checkAgentSource(
      "reviewer.md",
      GOOD_MD.replace("effort: xhigh", "effort: extreme"),
    ).join("\n"),
...
test("Codex format failures are named: unclosed block, missing field, bad name, unrecognized line", () => {
  assert.match(
    checkCodexAgentSource(
      "reviewer.toml",
      GOOD_TOML.replace(/\n"""\n$/, "\n"),
  ...
  assert.match(
    checkCodexAgentSource(
      "reviewer.toml",
      GOOD_TOML.replace('model = "gpt-5.6-sol"\n', ""),
    ).join("\n"),
    /missing required TOML field `model`/,
  );
  assert.match(
    checkCodexAgentSource("critic.toml", GOOD_TOML).join("\n"),
    /name `reviewer` != filename `critic`/,
  );
  assert.match(
    checkCodexAgentSource("reviewer.toml", `color = "red"\n${GOOD_TOML}`).join(
      "\n",
    ),
...
test("checkAgentsDir picks the checker by extension and refuses an unchecked directory", () => {
  ...
  try {
    ...
    assert.match(
      checkAgentsDir(claude).join("\n"),
      /broken\.md: no frontmatter fence/,
    );
    assert.deepEqual(checkAgentsDir(codex), []);
    assert.match(
      checkAgentsDir(empty)[0],
      /contains no \.md or \.toml agent definitions/,
    );
...
test("the builder seats must share one charter from Workflow: down", () => {
  ...
  writeFileSync(
    join(dir, "builder-lite.md"),
    seat("builder-lite", "2. only here\n"),
  );
  assert.match(
    checkBuilderParity(dir).join("\n"),
    /builder-lite\.md: shared builder charter .* differs from builder\.md/,
  );
  assert.equal(
    checkAgentsDir(dir).length,
    1,
    "the directory gate carries the parity problem",
  );
  writeFileSync(
    join(dir, "builder-lite.md"),
    "---\nname: builder-lite\ndescription: x\n---\nno workflow marker\n",
...
test("the reviewer charter must match across runtimes, sigil aside", () => {
  ...
  try {
    ...
    mkdirSync(claude);
    mkdirSync(codex);
    const body = (sigil, last) =>
      `You review the diff.\n- If math changed, flag that \`${sigil}validate-math\` must run.\n${last}\n`;
    writeFileSync(
      join(claude, "reviewer.md"),
      `---\nname: reviewer\ndescription: x\n---\n${body("/", "Report findings.")}`,
    );
    writeFileSync(
      join(codex, "reviewer.toml"),
      `name = "reviewer"\ndeveloper_instructions = """\n${body("$", "Report findings.")}"""\n`,
    );
    ...
    assert.match(
      problem,
      /reviewer\.md line 7 differs from .*reviewer\.toml line 5/,
    );
    assert.match(
      problem,
      /"Report findings\."; reviewer\.toml: "Fix findings\."/,
    );
    rmSync(join(codex, "reviewer.toml"));
    assert.match(
      checkReviewerParity(claude, codex)[0],
      /reviewer\.toml: reviewer charter is missing/,
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
...
test("the default CLI run refuses diverged reviewer charters", () => {
  ...
  try {
    ...
    writeFileSync(
      ...
      codex("You review the diff."),
    );
    assert.equal(
      spawnSync("node", [cli], { cwd: root, encoding: "utf8" }).status,
      0,
    );
    writeFileSync(
      join(root, ".codex", "agents", "reviewer.toml"),
      codex("You rewrite the diff."),
    );
