...
- **Acquired during delivery:** The pushed Object ID (OID), remote branch and pull-request identity, one recorded
  Greptile needed/skipped decision and, when needed, one settled current-head attempt, the applicable required check,
  merge evidence, tracker reconciliation, and closeout evidence.
...
3. Assess the complete pull-request diff under [Selective Greptile review](#selective-greptile-review). Record
   ...
   exhaustion records `UNAVAILABLE` and proceeds to step 4 without a request. When credits are available, add
   `independent-review` as review metadata and request `gh pr comment <PR_NUMBER> --repo zaingulel/RentCottage --body
   "@greptileai review this draft"`. Record the request URL, time, and exact `CURRENT_PR_HEAD`. Settle the attempt
   using the evidence below and reconcile every emitted finding before proceeding. No paid plan, billing change,
...
### Selective Greptile review

Use Greptile for consequential or difficult complete diffs: payment or booking integrity, concurrency,
authentication or permissions, privacy, migrations or destructive operations, significant integrations,
cross-system changes, or material uncertainty after internal review. Usually skip routine documentation and
instructions; copy, translations, and cosmetic changes; small isolated regression-proven fixes; mechanical verified
preservation; and test-only coverage-preserving changes. Risk and uncertainty override category and line count, so a
one-line permission change or a mixed diff still needs whole-change assessment. This is a judgement, not a score or
filename exemption; changes to legal meaning, access controls, security checks, or other consequential behaviour are
review-needed regardless of their routine category. A selective skip is neither `UNAVAILABLE` nor a clean review.

Required local evidence, independent review, owner approval, Continuous Integration (CI), merge protection, and
conversation resolution apply whether Greptile is needed or skipped.

When needed, Greptile is the sole external reviewer and the final review step for each pull-request head. `.greptile/config.json` disables automatic reviews; labels are metadata, and the explicit comment starts the review. Marking ready, pushing, and retrying CI must not request another review of an unchanged head.

Before pushing a same-outcome repair or rebase, keep the pull request in draft (`gh pr ready <PR_NUMBER> --repo
zaingulel/RentCottage --undo` if already ready). Complete focused verification, commit the change, and complete the
scoped local repair review required by `AGENTS.md` before pushing. Any changed head invalidates the old
classification, Greptile and CI evidence: record the new head and reassess the whole diff. If Greptile is newly or
still needed, recheck current allowance and follow its available/exhausted handling; request `@greptileai review this
draft again; <what changed> in <commit>` when available. If it is skipped, record the new reason. Rebase before the
final required Greptile attempt, not
...
When Greptile is needed, record exactly one settled state for the current head:
...
Complete the original delivery packet with the pull request and merged commit, internal review, current-head Greptile
needed/skipped decision and state when needed, source-bound `test`, conversation resolution, hosted-setting readbacks
when applicable, tracker reconciliation, board verification, closeout result, every retained target, and every
unavailable or skipped observation.

Recurring maintenance is the applicable local review, one best-effort Greptile attempt when needed, ordinary GitHub
state reads, and native closeout. There is no release command, cleanup service, or additional workflow state store.
