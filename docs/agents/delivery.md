...
3. **Fresh exact-head internal review:** `head=CURRENT_PR_HEAD`; run `security-code-review` against the complete exact pushed head bundle. `UNKNOWN` stops. Use entirely fresh Standards and Specification reviewers, plus a fresh Security reviewer when the aggregate is `ANY_YES`; every required verdict and finding disposition must be current for `CURRENT_PR_HEAD`. Pre-outward review verdicts are ineligible for exact-head quality.
4. **Attempt final Greptile review:** `after=fresh exact-head internal review`; `head=CURRENT_PR_HEAD`; request the final exact-head review step from Greptile and settle the attempt as `COMPLETE` or `UNAVAILABLE` using the evidence below. No paid plan, billing change, purchase, or upgrade is authorised. No internal review follows a settled Greptile attempt on the same head.
5. **Exact-head quality:** `after=settled Greptile attempt`; `head=CURRENT_PR_HEAD`; only after the fresh required internal reviews, required local verification, and every finding emitted by a completed or unavailable Greptile attempt has an evidence-based disposition, explicitly dispatch `quality` from trusted `main` with `gh workflow run ci.yml --repo zaingulel/RentCottage --ref main -f pull_request_number=<PR_NUMBER> -f expected_head_oid=<CURRENT_PR_HEAD>`.

Greptile is the sole external reviewer and the final review step for each pull-request head. The `independent-review` label is its trigger and evidence label, not a route selector. It follows the required internal review and local verification; exact-head Continuous Integration and merge-state checks follow it. A completed state proves only that processing finished, not that the change is correct or approved.
...
Missing, unknown, stale, or unattributed attempt evidence stops delivery. Self-authored, wrong-provider, missing, untrusted, or unattributed artifacts cannot be `COMPLETE` and stop delivery. A push invalidates Greptile, internal-review, and Continuous Integration evidence. Every repair uses an ordinary Git commit and `git push origin refs/heads/<LOCAL_TOPIC_BRANCH>:refs/heads/<PR_HEAD_BRANCH>`. It then restarts `security-code-review` classification and entirely new internal reviewers before a fresh exact-head Greptile attempt when allowance permits, or a new exact-head `UNAVAILABLE` record otherwise, followed by exact-head quality. Never carry any of this evidence across heads.
