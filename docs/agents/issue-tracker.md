...
Exit statuses are `0` for a successful apply, `5` for a verified no-op, `2` for invalid arguments, `3` when a dry-run has proposed changes, `4` for incomplete or contradictory evidence, and `1` for an execution or final-verification failure. The scheduled check accepts `5` as its clean state; every drift or failure state remains non-zero. Output is bounded JSON and never contains credentials.
