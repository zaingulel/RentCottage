...
Pull-request handling, Greptile review, GitHub Actions, and GitHub repository rules remain separate authorities. Greptile is attempted against the exact current head without authorising paid service, and its settled unavailable state does not weaken any mandatory internal, executable, Continuous Integration, conversation, ownership, tracker, merge or release gate. A completed state means processing finished only; the coordinator reconciles every current-head finding before marking the pull request ready. Every push invalidates the earlier Greptile and Continuous Integration evidence; local repair review remains scoped to the repaired hunks and what they could break. A ready pull request runs GitHub Actions against its merge result and exposes the single required, source-bound `test` check. The hosted `main` ruleset requires that check with strict current-base enforcement and conversation resolution; GitHub auto-merge queues the approved squash merge until those protections pass. Changes to either hosted setting remain explicit owner-gated actions.
