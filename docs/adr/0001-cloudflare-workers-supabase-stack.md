...
- Graphite as the pull-request interface, with Graphite AI review or Greptile selected exclusively for additional automated-review evidence.
...
Graphite stack management, external AI review, GitHub Actions, and GitHub repository rules remain separate authorities. The repository selects exactly one external AI reviewer: Greptile for sensitive, delivery-integrity, or explicitly escalated changes, and Graphite AI review for ordinary changes. A provider's completed state means processing finished only; the coordinator reconciles every selected-provider current-head finding before explicitly dispatching exact-head `quality` from trusted `main`. Every push invalidates the earlier review and Continuous Integration evidence. GitHub Actions performs executable verification. The hosted GitHub repository ruleset must require `quality` from the observed GitHub Actions source, application or integration, not the `quality` check name alone, and must enforce conversation resolution; changing that hosted ruleset remains an explicit owner-gated action.
