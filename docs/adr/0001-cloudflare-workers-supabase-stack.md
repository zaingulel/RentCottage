...
Payment, translation, and notification suppliers will sit behind narrow application interfaces. Qi Card is the first payment candidate to validate, but no payment provider is selected until it demonstrates the complete required flow in a sandbox and contract.
...
- Static interface translations require human review. Automatic translation of approved dynamic content must preserve the original text and use a replaceable translation service.
...
- Provider-specific code remains replaceable while Iraqi payment and translation suppliers are validated.
