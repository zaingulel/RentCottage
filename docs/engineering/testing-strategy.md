...
- `npm run verify:access` runs a bounded Chromium launch preflight, then the database, policy, concurrency, browser-backed access, and scheduled-expiry evidence in that order. `npm run verify:access:database` reruns only database, policy, and concurrency evidence; `npm run verify:access:browser` reruns only the Chromium preflight and browser-backed access evidence against an already prepared local database. Browser-only retries deliberately do not reset the database or rerun fixture-contract, concurrency, or scheduled-expiry evidence.
