...
- `npm run verify` is the local and continuous-integration gate. It audits production dependencies, checks formatting, lint and strict TypeScript, runs Vitest, builds and smoke-tests the Worker, scans browser assets for secrets, verifies generated Cloudflare types, and runs the current browser journeys.
