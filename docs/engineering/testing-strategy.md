...
- `npm run verify:access` runs the Chromium preflight, database, policy, concurrency, browser-backed access, and scheduled-expiry evidence in that order. `npm run verify:access:database` runs only database, fixture-contract, policy, and concurrency evidence. `npm run verify:access:browser` is self-contained: after the Chromium preflight it starts the named disposable local Supabase stack, creates and validates only its browser prerequisites, and runs browser-backed access evidence. After any successful stack start, cleanup uses `supabase stop --no-backup`, so completed or downstream-failed invocations do not hand retained database or Storage state to another mode. A normal browser-only retry rebuilds its browser prerequisites without running `db reset`, fixture-contract, policy/concurrency, or scheduled-expiry evidence.
