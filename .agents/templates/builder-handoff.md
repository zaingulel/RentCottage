...
- Work only in the working directory above. Verify ONLY this claim with the focused verification command; NEVER
  run the full suite, that is the coordinator's job after the claim converges.
...
- Mutation-proofing by revert is the coordinator's convergence step, never yours.
