...
2. Read the current allowance first: Greptile's usage page for the `flowgauge` organisation, which RentCottage
   ...
   Greptile's summary is a `greptile-apps[bot]` comment whose footer names the exact requested head as `Last
   reviewed commit`, and the `Greptile Review` check turning green is a second signal; its heading is an image,
   so a watcher keys on that footer, never on heading text.
