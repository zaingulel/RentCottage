...
select is(jsonb_array_length(public.get_booking_financial_view('RC-REQ-0000000000001001','customer')->'notifications'),3,'participant receives only their own confirmation, reminder, and cancellation notice states');
select is((select notice->>'recipientRole' from jsonb_array_elements(public.get_booking_financial_view('RC-REQ-0000000000001001','customer')->'notifications') notice where notice->>'kind'='preparation_reminder'),'customer','customer financial history identifies only their reminder recipient role');
select ok((select (notice ? 'dueAt') and (notice ? 'state') and (notice ? 'retryAllowed') from jsonb_array_elements(public.get_booking_financial_view('RC-REQ-0000000000001001','customer')->'notifications') notice where notice->>'kind'='preparation_reminder'),'customer financial history exposes the reminder deadline and operational state');
select set_config('request.jwt.claim.sub','10000000-0000-4000-8000-000000003801',true);
select set_config('request.jwt.claims','{"sub":"10000000-0000-4000-8000-000000003801","aal":"aal2"}',true);
