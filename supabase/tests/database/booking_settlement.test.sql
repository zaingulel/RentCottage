...
begin
  ...
  recorder:=case admission->>'purpose' when 'booking-settlement' then 'record_booking_settlement_observation' when 'booking-request-capture' then 'record_booking_request_capture_observation'
...
select throws_ok($$select public.request_booking_settlement('60000000-0000-4000-8000-000000001001','90000000-0000-4000-8000-000000002280','Settlement review','stale',99000000)$$,'RC409',null,'date alone cannot create a settlement without authoritative maturity');
reset role;
set local role service_role;
...
select throws_ok($$select public.request_booking_settlement('60000000-0000-4000-8000-000000001001','90000000-0000-4000-8000-000000002280','Wrong commission',public.get_booking_settlement_facts('60000000-0000-4000-8000-000000001001')->>'revision',103500000)$$,'RC409',null,'commission is on booking price, never customer total');
...
select throws_ok($$select public.request_booking_settlement('60000000-0000-4000-8000-000000001001','90000000-0000-4000-8000-000000002280','Settlement review',public.get_booking_settlement_facts('60000000-0000-4000-8000-000000001001')->>'revision',99000000)$$,'RC409',null,'refund reservation first prevents settlement request');
rollback to refund_first;
insert into settlement_values values('intent',public.request_booking_settlement('60000000-0000-4000-8000-000000001001','90000000-0000-4000-8000-000000002280','Settlement review',public.get_booking_settlement_facts('60000000-0000-4000-8000-000000001001')->>'revision',99000000));
select is(public.request_booking_settlement('60000000-0000-4000-8000-000000001001','90000000-0000-4000-8000-000000002280','Settlement review','stale',99000000),pg_temp.settlement_value('intent'),'same command replays original intent');
select throws_ok($$select public.request_booking_settlement('60000000-0000-4000-8000-000000001001','90000000-0000-4000-8000-000000002280','Changed reason','stale',99000000)$$,'RC409',null,'replay binds attributed command content');
select pg_temp.actor('10000000-0000-4000-8000-000000003801','aal1');
select throws_ok($$select public.request_booking_settlement('60000000-0000-4000-8000-000000001001','90000000-0000-4000-8000-000000002280','Settlement review','stale',99000000)$$,'42501',null,'replay requires current AAL2');
select pg_temp.actor('10000000-0000-4000-8000-000000003801','aal2');
reset role;
savepoint revoked;
update public.account_contexts set role='customer' where user_id='10000000-0000-4000-8000-000000003801';
set local role authenticated;
select throws_ok($$select public.request_booking_settlement('60000000-0000-4000-8000-000000001001','90000000-0000-4000-8000-000000002280','Settlement review','stale',99000000)$$,'42501',null,'revoked administrator cannot replay');
rollback to revoked;
set local role service_role;
select throws_ok($$select public.request_booking_settlement('60000000-0000-4000-8000-000000001001','90000000-0000-4000-8000-000000002280','Settlement review','stale',99000000)$$,'42501',null,'service role cannot impersonate attributed administrator command');
set local role service_role;
...
select throws_ok($$select public.admit_booking_settlement(jsonb_set(pg_temp.settlement_value('claim')#>'{request,executionPermit}','{binding,amountFils}','103500000'))$$,'RC409',null,'changed amount cannot admit');
