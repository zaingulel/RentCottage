...
create function public.booking_request_marketplace_terms(
...
revoke all on function public.booking_request_marketplace_terms(
  public.cottage_profile_source_language
...
create table public.booking_request_submission_attempts (
  ...
  state text not null check (state in (
    ...
    'reconciliation_required', 'releasing', 'released', 'expired', 'finalized'
  )),
...
create type public.booking_request_authorization_claim_state as enum (
  ...
  'releasing', 'released', 'expired', 'converted'
);
...
create table public.booking_request_authorization_claims (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  check (reconciliation_expires_at <= not_after),
...
create table public.simulated_payment_provider_operations (
  id uuid primary key,
  claim_id uuid not null,
  claim_generation integer not null,
  operation_kind text not null check (operation_kind in ('authorization', 'release')),
  ...
  original_outcome text not null
    check (original_outcome in ('succeeded', 'failed', 'indeterminate')),
  current_outcome text not null
    check (current_outcome in ('succeeded', 'failed', 'indeterminate')),
  provider_request_id text not null,
  provider_reference text not null,
  movement_reference text,
  physical_execution_count smallint not null default 1
    check (physical_execution_count = 1),
...
create table public.booking_snapshots (
  ...
  booking_terms_sha256 text not null
    check (booking_terms_sha256 ~ '^[0-9a-f]{64}$'),
...
begin
  ...
  if target_attempt.state = 'expired' then
    return jsonb_build_object('status', 'unavailable');
  end if;
  if exists (
    select 1
...
create function public.execute_simulated_payment_provider_operation(
...
begin
  if current_setting('role', true) <> 'service_role'
    ...
    or (select count(*) from jsonb_object_keys(target_operation)) <> 11
    or not (target_operation ?& array[
      'providerIdentity', 'idempotencyKey', 'requestFingerprint',
      'paymentLifecycleId', 'logicalOperationId', 'physicalAttemptId',
      'operationKind', 'amountFils', 'currency', 'claimId', 'claimGeneration'
    ])
    or provider_identity is null
    or (select count(*) from jsonb_object_keys(provider_identity)) <> 4
    or provider_identity ->> 'environment' <> 'local-test'
    or coalesce(provider_identity ->> 'provider', '') = ''
    or coalesce(provider_identity ->> 'merchantId', '') = ''
    or coalesce(provider_identity ->> 'terminalId', '') = ''
    or target_operation ->> 'operationKind' not in ('authorization', 'release')
    or target_operation ->> 'currency' <> 'IQD'
    or target_operation ->> 'requestFingerprint' !~ '^[0-9a-f]{64}$' then
  ...
  if not found
    ...
    raise exception 'Fictional provider operation does not match its claim'
      using errcode = 'RC409';
  end if;

  if target_operation ->> 'operationKind' = 'authorization' then
    if (target_operation ->> 'claimId')::uuid <> claim.id
      or (target_operation ->> 'claimGeneration')::integer <> claim.generation
      or target_operation ->> 'idempotencyKey'
        <> claim.provider_idempotency_key
      or target_operation ->> 'logicalOperationId' <> claim.logical_operation_id
      or target_operation ->> 'physicalAttemptId' <> claim.physical_attempt_id
    ...
    if target_operation -> 'claimId' <> 'null'::jsonb
      ...
      or target_operation -> 'idempotencyKey' <> 'null'::jsonb
      ...
      or claim.state not in ('authorized', 'releasing', 'reconciliation_required') then
      raise exception 'Fictional release operation is invalid'
        using errcode = 'RC409';
    end if;
    effective_idempotency_key := claim.provider_idempotency_key || ':release:'
...
create function public.query_simulated_payment_provider_operation(
  target_operation jsonb,
...
begin
  if current_setting('role', true) <> 'service_role'
    ...
    or target_operation ->> 'operationKind' not in ('authorization', 'release')
    or target_operation ->> 'currency' <> 'IQD'
    or target_operation ->> 'requestFingerprint' !~ '^[0-9a-f]{64}$'
...
$$;
revoke all on function public.query_simulated_payment_provider_operation(
  jsonb, text, text, text
) from public, anon, authenticated;
grant execute on function public.query_simulated_payment_provider_operation(
  jsonb, text, text, text
) to service_role;

create function public.expire_booking_request_authorization_claims()
returns integer
language plpgsql
volatile
security definer
...
$$;
revoke all on function public.expire_booking_request_authorization_claims()
  from public, anon, authenticated;
grant execute on function public.expire_booking_request_authorization_claims()
  to service_role;
