...
REVOKE ALL ON FUNCTION public.get_customer_review(text) FROM PUBLIC;

GRANT ALL ON FUNCTION public.get_customer_review(text) TO authenticated;

CREATE FUNCTION public.hide_customer_review (
...
REVOKE ALL ON FUNCTION public.hide_customer_review(uuid, text) FROM PUBLIC;

GRANT ALL ON FUNCTION public.hide_customer_review(uuid, text) TO authenticated;
...
REVOKE ALL ON FUNCTION public.list_administrator_customer_reviews(timestamp WITH time zone, uuid, integer) FROM PUBLIC;

GRANT ALL ON FUNCTION public.list_administrator_customer_reviews(timestamp WITH time zone, uuid, integer) TO authenticated;
...
REVOKE ALL ON FUNCTION public.list_public_customer_reviews(text, timestamp WITH time zone, uuid, integer) FROM PUBLIC;

GRANT ALL ON FUNCTION public.list_public_customer_reviews(text, timestamp WITH time zone, uuid, integer) TO anon;

GRANT ALL ON FUNCTION public.list_public_customer_reviews(text, timestamp WITH time zone, uuid, integer) TO authenticated;

CREATE FUNCTION public.reject_customer_review_fact_change()
  RETURNS TRIGGER
  LANGUAGE plpgsql
  SET search_path TO ''
  AS $function$
...
REVOKE ALL ON FUNCTION public.reject_customer_review_fact_change() FROM PUBLIC;

CREATE FUNCTION public.submit_customer_review (
  target_reference         text,
...
REVOKE ALL ON FUNCTION public.submit_customer_review(text, integer, public.cottage_profile_source_language, text) FROM PUBLIC;

GRANT ALL ON FUNCTION public.submit_customer_review(text, integer, public.cottage_profile_source_language, text) TO authenticated;

CREATE TABLE public.customer_review_hides (
...
ALTER TABLE public.customer_review_hides
  ENABLE ROW LEVEL SECURITY;

ALTER TABLE public.customer_review_hides
  ADD CONSTRAINT customer_review_hides_administrator_user_id_fkey FOREIGN KEY (administrator_user_id) REFERENCES auth.users(id) ON DELETE RESTRICT;

ALTER TABLE public.customer_review_hides
  ADD CONSTRAINT customer_review_hides_pkey PRIMARY KEY (review_id);

ALTER TABLE public.customer_review_hides
  ADD CONSTRAINT customer_review_hides_reason_check CHECK (char_length(btrim(reason)) >= 1 AND char_length(btrim(reason)) <= 2000);

CREATE TRIGGER customer_review_hides_immutable
  BEFORE DELETE OR UPDATE ON public.customer_review_hides
  FOR EACH ROW
...
ALTER TABLE public.customer_reviews
  ADD CONSTRAINT customer_reviews_author_user_id_fkey FOREIGN KEY (author_user_id) REFERENCES auth.users(id) ON DELETE RESTRICT;

ALTER TABLE public.customer_reviews
  ADD CONSTRAINT customer_reviews_booking_confirmation_id_fkey FOREIGN KEY (booking_confirmation_id) REFERENCES public.booking_confirmations(id) ON DELETE RESTRICT;

ALTER TABLE public.customer_reviews
  ADD CONSTRAINT customer_reviews_booking_request_id_fkey FOREIGN KEY (booking_request_id) REFERENCES public.booking_requests(id) ON DELETE RESTRICT;

ALTER TABLE public.customer_reviews
  ADD CONSTRAINT customer_reviews_booking_request_id_key UNIQUE (booking_request_id);

ALTER TABLE public.customer_reviews
  ADD CONSTRAINT customer_reviews_original_body_check
    CHECK (original_body IS NULL OR char_length(original_body) >= 1 AND char_length(original_body) <= 2000 AND char_length(btrim(original_body)) >= 1);

ALTER TABLE public.customer_reviews
  ADD CONSTRAINT customer_reviews_pkey PRIMARY KEY (id);

ALTER TABLE public.customer_review_hides
  ADD CONSTRAINT customer_review_hides_review_id_fkey FOREIGN KEY (review_id) REFERENCES public.customer_reviews(id) ON DELETE RESTRICT;

ALTER TABLE public.customer_reviews
  ADD CONSTRAINT customer_reviews_profile_id_fkey FOREIGN KEY (profile_id) REFERENCES public.owner_application_cottage_profiles(id) ON DELETE RESTRICT;

ALTER TABLE public.customer_reviews
  ADD CONSTRAINT customer_reviews_rating_check CHECK (rating >= 1 AND rating <= 5);
...
CREATE TRIGGER customer_reviews_immutable
  BEFORE DELETE OR UPDATE ON public.customer_reviews
  FOR EACH ROW
  EXECUTE FUNCTION public.reject_customer_review_fact_change();
